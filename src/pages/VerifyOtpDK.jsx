...
const VerifyCode = () => {
      ...
      if (emailStored) {
        signInWithEmailLink(authentication, emailStored, window.location.href)
          .then((result) => {
            ...
            //window.localStorage.removeItem("sdt"); // Xóa số điện thoại khỏi localStorage

            // Chuyển hướng sang trang đổi mật khẩu
            setTimeout(() => {
              navigate("/signup-info", { state: { email: emailStored ,sdt:sdtStored} });
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-[#DEF7FF]">
      ...
      <p className="text-gray-600">Đăng ký tài khoản cho Echo</p>
